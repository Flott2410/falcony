  ...
  def perform
    ...
    countries.each do |country|
      ...
      cases[country[0]]['data'].each do |covid_data|
        ...
        # only update cases where the date for the API entries are superior to the last case for the current country
        if date > Case.where(country: Country.find_by(iso: country[0])).last.date
          Case.create!(
            date: date,
